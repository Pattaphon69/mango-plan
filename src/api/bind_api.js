...
export const readPicture = async (docno) => {
  return await $xt.getServer(`Anywhere/Center/AttachPictures?docno=${docno}&doctype=OTH&lineno=0`);
};
...
export const postCancelSubmit = async (docno,module) => {
  // console.log("obj", obj);
  return await $xt.getServer(`anywhere/center/cancelsubmitapprove?module=${module ||"OTH"}&docno=${docno}`);
};
...
export const getFormSignature = async () => {
  return await $xt.getServer(`anywhere/center/FormSignature?windowcode=IC_EN&module=IC_EN`);
};
...
export const getICAPP_ReadList = async (pre_event2, docno) => {
  console.log("pre_event2, docno", pre_event2, docno);
...
export const pr_getRetrieve = async () => {
  return await $xt.getServer(`Anywhere/OF/PR_ReadList?skip=0&take=99999&sort=prno&sort_type=desc&pending=N&field=prno`);
};
...
export const getLocation = async (lat, lng) => {
  return axios
    .get(`http://203.154.41.4/mango_geo/data/fetch`, {
      params: {
        lat,
        lng,
      },
...
export const postUpdateIssueFromMobile = async (data) => {
  // console.log("DATA", data);
  return await $xt.postServerJson(`Anywhere/ic/UpdateIssueFromMobile`, data);
};
